...
@Injectable()
export class DynamicPharmFormResolver implements Resolve<FieldConfig>{
  ...
  resolve(): Observable<FieldConfig> {
    this.idSubject = this.dynamicFormService.getPharmIdSubject();

    /** if someone remove  idPharmSubject from localStorage**/
    if(!this.idSubject) {
      console.error('idPharmSubject is null in localStorage!');
